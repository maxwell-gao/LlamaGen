...
from autoregressive.models.generate import (
    generate,
)  # 虽然此处用不到，但保留原始文件的导入
# ----------------------------------------
...
class ProtoTokenOptimizer:
    ...
    def __init__(self, model, hidden_size, device, num_steps=1000, use_wandb=False):
        ...
        self.e_t = nn.Parameter(
            torch.randn(1, hidden_size, device=device, dtype=self.dtype) * 0.01
        )
    ...
    def construct_input_embeddings(self, seq_length):
        ...
        return input_embeds.unsqueeze(0).to(
    ...
    def train_step(self, target_tokens):
        ...
        # Gradient clipping
        torch.nn.utils.clip_grad_norm_([self.e_t, self.m], max_norm=2.0)
...
def get_reference_tokens(vq_model, gpt_model, args):
    ...
    with torch.inference_mode():
        index_for_decode = index_sample.reshape(-1, latent_size, latent_size).unsqueeze(
            1
        )
...
def verify_proto_tokens(model, visual_tokens, args, vq_model=None, latent_size=None):
    ...
    if hasattr(model, "config"):
        hidden_size = getattr(model.config, "hidden_size", None) or getattr(
            model.config, "dim", None
        )
    else:
        hidden_size = getattr(model, "embed_dim", None) or getattr(
            model, "d_model", None
        )
    if hidden_size is None:
        raise RuntimeError(
            "Could not determine model hidden size (expected model.config.dim or model.embed_dim)"
        )
    ...
    for step in pbar:
        ...
        if getattr(args, "eval_interval", 0) > 0 and (
            (step + 1) % args.eval_interval == 0
        ):
            # Run evaluation to collect metrics
            eval_loss, eval_acc, eval_correct, eval_top5 = optimizer.evaluate(
                visual_tokens
            )
            print(
                f"[Eval @ step {step + 1}] Loss: {eval_loss:.4f}  Acc: {eval_acc:.2f}% ({eval_correct}/{seq_length})"
            )
            ...
            if (
                ...
                try:
                    recon_image, recon_tokens = reconstruct_image(
                    ...
                    recon_path = os.path.join(
                        args.save_dir, f"reconstructed_image_step_{step + 1}.png"
                    )
                    save_image(
                        recon_image, recon_path, normalize=True, value_range=(-1, 1)
                    )
                    print(f"Saved eval reconstructed image to: {recon_path}")
                except Exception as e:
                    print(
                        f"Warning: failed to reconstruct/upload eval image at step {step + 1}: {e}"
                    )
...
def reconstruct_image(optimizer, vq_model, visual_tokens, latent_size, args):
    ...
    with torch.inference_mode():
        reconstructed_image = vq_model.decode_code(
            index_sample, shape=[1, args.codebook_embed_dim, latent_size, latent_size]
        )
...
def main(args):
    ...
    if os.path.exists(tokens_path) and not args.force_generate:
        ...
        with torch.inference_mode():
            reference_image = vq_model.decode_code(
                index_sample,
                shape=[
        ...
        wandb.log(
            {
                "target_image": wandb.Image(
                    ref_img_vis, caption="Target Image (Visualization of Target Tokens)"
                ),
    ...
    optimizer = verify_proto_tokens(
        gpt_model, visual_tokens, args, vq_model=vq_model, latent_size=latent_size
    )
        ...
        wandb.log(
            {
                ...
                "comparison": wandb.Image(
                    comp_img_vis, caption="Left: Target | Right: Reconstructed"
                ),
            }
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify Proto-Tokens Feasibility for LlamaGen-like GPT"
    )

    # --- LlamaGen Model related (based on sample.py) ---
    parser.add_argument(
        "--gpt-model", type=str, choices=list(GPT_models.keys()), default="GPT-B"
    )
    parser.add_argument(
        "--gpt-ckpt", type=str, required=True, help="ckpt path for GPT model"
    )
    parser.add_argument(
        "--gpt-type",
        type=str,
    ...
    parser.add_argument(
        "--cfg-interval", type=int, default=-1, help="classifier-free guidance interval"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Class id to condition generation on (default: first from sample list)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Evaluation interval (steps). When >0, run evaluation and upload reconstructed_image each interval",
    )
    ...
    parser.add_argument(
        "--force-generate",
        action="store_true",
        help="Force generate/re-encode target tokens",
    )
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default="./proto_tokens_exp_llama",
        help="Directory to save results",
    )
    parser.add_argument(
        "--device", type=str, default="cuda", help="Computing device (Default is cuda)"
    )
    ...
    if unknown:
        ...
        extra_parser.add_argument(
            "--cfg-scale", type=float, default=getattr(args, "cfg_scale", 4.0)
        )
        extra_parser.add_argument(
            "--cfg-interval", type=int, default=getattr(args, "cfg_interval", -1)
        )
        extra_parser.add_argument(
            "--top-k", type=int, default=getattr(args, "top_k", 2000)
        )
        extra_parser.add_argument(
            "--temperature", type=float, default=getattr(args, "temperature", 1.0)
        )
        extra_parser.add_argument(
            "--top-p", type=float, default=getattr(args, "top_p", 1.0)
        )
        ...
        for k, v in vars(extra_args).items():
            setattr(args, k, v)
    args.device = (
        "cuda" if torch.cuda.is_available() and args.device == "cuda" else args.device
    )
